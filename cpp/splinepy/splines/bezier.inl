...
typename Bezier<para_dim>::Base_ Bezier<para_dim>::CreateBase(const int* degrees,
  ...
  for (std::size_t i{}; i < ncps * dim; ++i) {
    bm_vertices.push_back(control_points[i]);
  }

  assert(ncps == bm_control_points.GetNumberOfVertices());
...
template<std::size_t para_dim>
constexpr auto
Bezier<para_dim>::operator()(const ParametricCoordinate_& query) const {
...
/// @param order
template<std::size_t para_dim>
constexpr auto Bezier<para_dim>::operator()(const ParametricCoordinate_& query,
...
/// @param p_dim
template<std::size_t para_dim>
constexpr auto Bezier<para_dim>::ElevateDegree(const Dimension_ p_dim) {
  return Base_::OrderElevateAlongParametricDimension(p_dim);
}

template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyCurrentProperties(
    int* degrees,
  ...
  // control_points
  if (control_points) {
    const auto& vertices = Base_::control_points.GetVertices();
    const auto vertices_size = vertices.size();
    std::copy_n(vertices.data(), vertices_size, control_points);
  }
}

template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyParametricBounds(double* para_bounds) const {
  for (std::size_t i{}; i < kParaDim; ++i) {
    // lower bounds
    para_bounds[i] = 0.;
    // upper bounds
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyControlMeshResolutions(
    int* control_mesh_res) const {
  const auto cm_res = helpers::GetControlMeshResolutions(*this);
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyGrevilleAbscissae(double* greville_abscissae,
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyEvaluate(const double* para_coord,
                                        double* evaluated) const {
  helpers::ScalarTypeEvaluate(*this, para_coord, evaluated);
}

template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyDerivative(const double* para_coord,
                                          const int* orders,
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyJacobian(const double* para_coord,
                                        double* jacobians) const {
  helpers::ScalarTypeJacobian(*this, para_coord, jacobians);
}

template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyPlantNewKdTreeForProximity(
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyVerboseProximity(
    const double* query,
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyElevateDegree(const int& p_dim) {
  helpers::ScalarTypeElevateDegree(*this, p_dim);
}

template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyBasis(const double* para_coord,
                                     double* basis) const {
  helpers::BezierBasis(*this, para_coord, basis);
}

template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyBasisDerivative(const double* para_coord,
                                               const int* order,
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepySupport(const double* para_coord,
                                       int* support) const {
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyBasisAndSupport(const double* para_coord,
                                               double* basis,
                                               int* support) const {
...
template<std::size_t para_dim>
void Bezier<para_dim>::SplinepyBasisDerivativeAndSupport(
...
template<std::size_t para_dim>
std::shared_ptr<SplinepyBase> Bezier<para_dim>::SplinepyMultiply(
  ...
  // good to multiply
  if (a->SplinepyDim() == 1) {
    auto true_a = static_cast<Bezier<para_dim>&>(*a);
    return std::make_shared<Bezier<para_dim>>(this->Base_::operator*(true_a));
  } else {
    auto true_a = static_cast<Bezier<para_dim>&>(*a);
    return std::make_shared<Bezier<para_dim>>(this->Base_::operator*(true_a));
  }
...
Bezier<para_dim>::SplinepyAdd(const std::shared_ptr<SplinepyBase>& a) const {
  SplinepySplineNameMatches(
      *this,
      *a,
  ...
  return std::make_shared<Bezier<para_dim>>(
      this->Base_::operator+(static_cast<Bezier<para_dim>&>(*a)));
}
...
Bezier<para_dim>::ConvertComposeToBase(
  ...
  if constexpr (is_rational) {
    ...
    for (size_t i{}; i < super_return.size(); ++i) {
      to_return[i] = std::make_shared<Bezier<inner_para_dim>>(super_return[i]);
    }
    return to_return;
  }
}
...
template<std::size_t para_dim>
std::vector<std::shared_ptr<SplinepyBase>>
Bezier<para_dim>::SplinepyComposeSensitivities(
    const std::shared_ptr<SplinepyBase>& inner_function) const {
  // type check
...
///   2. dim is same as outer_function's par_dim
template<std::size_t para_dim>
std::shared_ptr<SplinepyBase> Bezier<para_dim>::SplinepyCompose(
    const std::shared_ptr<SplinepyBase>& inner_function) const {
  // type check
  if (inner_function->SplinepySplineName().find("Bezier")
      == std::string::npos) {
  ...
  if (inner_function->SplinepyIsRational()) {
    switch (inner_function->SplinepyParaDim()) {
    case 1:
      return std::make_shared<RationalBezier<1>>(this->Compose(
          static_cast<const RationalBezier<1>&>(*inner_function)));
       case 2:
        return std::make_shared<RationalBezier<2>>(this->Compose(
            static_cast<const RationalBezier<2>&>(*inner_function)));
      case 3:
        return std::make_shared<RationalBezier<3>>(this->Compose(
            static_cast<const RationalBezier<3>&>(*inner_function))); 
#ifdef SPLINEPY_MORE
    case 4:
...
template<std::size_t para_dim>
std::vector<std::shared_ptr<SplinepyBase>>
Bezier<para_dim>::SplinepySplit(const int& p_dim,
  ...
  for (std::size_t i{}; i < n_splitted; ++i) {
    splitted.emplace_back(std::make_shared<Bezier<para_dim>>(bm_splitted[i]));
  }
...
template<std::size_t para_dim>
std::shared_ptr<SplinepyBase>
Bezier<para_dim>::SplinepyDerivativeSpline(const int* orders) const {
  ...
  return std::make_shared<Bezier<para_dim>>(derived_bez);
}

template<std::size_t para_dim>
std::vector<std::shared_ptr<SplinepyBase>>
Bezier<para_dim>::SplinepyExtractBezierPatches() const {
  // should copy
  return {std::make_shared<Bezier<para_dim>>(*this)};
}

template<std::size_t para_dim>
std::shared_ptr<SplinepyBase>
Bezier<para_dim>::SplinepyExtractBoundary(const int& boundary_id) {
...
template<std::size_t para_dim>
std::shared_ptr<SplinepyBase>
Bezier<para_dim>::SplinepyExtractDim(const int& phys_dim) const {
  return std::make_shared<Bezier<para_dim>>(
...
template<std::size_t para_dim>
std::shared_ptr<SplinepyBase> Bezier<para_dim>::SplinepyCompositionDerivative(
...
template<std::size_t para_dim>
constexpr void Bezier<para_dim>::CreateProximityIfNull() {
  if (!proximity_) {
    switch (SplinepyDim()) {
    case 1:
...
template<std::size_t para_dim>
constexpr typename Bezier<para_dim>::ProximityBase_& Bezier<para_dim>::GetProximity() {
  CreateProximityIfNull();
  return *proximity_;
}
/// @brief Gets proximity
template<std::size_t para_dim>
constexpr const typename Bezier<para_dim>::ProximityBase_& Bezier<para_dim>::GetProximity() const {
  return *proximity_;
}
