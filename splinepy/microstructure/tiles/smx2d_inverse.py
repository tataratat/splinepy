...
class SMX2DInverse(_TileBase):
    ...
    _closure_directions = ["x_min", "x_max"]
    ...
    def _closing_tile(
        ...
        **kwargs,  # noqa ARG002
        ...
        parameters: np.ndarray
            Parameters corresponding to corner thicknesses
        parameter_sensitivities: np.ndarray
            Parameter sensitivities
        closure: str
            Parametric dimension to be closed

        Returns
        --------------
        (splines, derivatives): tuple<list<Spline>, list<list<Spline>>>
        ...
        for i_derivative in range(n_derivatives + 1):
            ...
            # Define common patches for all closure directions
            control_points_list = [
                sw_bar_bottom,
                sw_bar_top,
                se_bar_bottom,
            ...
            # For closure, replace corresponding control points in the list
            if closure == "x_min":
                # Modify triangle_w_s
                control_points_list[11][0, 1] = v_zero
                # Modify sw_bar_bottom
                control_points_list[0][2, 1] = v_zero
                # triangle_w_n
                control_points_list[12][2, 1] = v_one
                # nw_bar_top
                control_points_list[4][2, 1] = v_one
            elif closure == "x_max":
                # triangle_e_s
                control_points_list[14][1, 1] = v_zero
                # se_bar_bottom
                control_points_list[2][1, 1] = v_zero
                # triangle_e_n
                control_points_list[15][3, 1] = v_one
                # ne_bar_top
                control_points_list[7][3, 1] = v_one
            else:
                raise NotImplementedError(
