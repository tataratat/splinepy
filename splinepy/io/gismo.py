...
def export(fname, multipatch=None, indent=True):
    ...
    patch_range.text = (
            f"{index_offset} "
            f"{len(multipatch.splines) - 1 + index_offset}"
    )
    ...
    if con_spline_id.size == 0:
        ...
        is_lower_id = multipatch.interfaces[con_spline_id,
                                            con_face_id] < con_spline_id
        ...
        start_order = np.lexsort(
                (
                        con_spline_id_start,
                        multipatch.interfaces[con_spline_id_start,
                                              con_face_id_start]
                )
        ).reshape(-1, 1)
        end_order = np.lexsort(
                (
                        multipatch.interfaces[con_spline_id_end,
                                              con_face_id_end],
                        con_spline_id_end
                )
        ).reshape(-1, 1)
        ...
        interface_array = np.hstack(
                (
                        ...
                        np.ones(
                                (start_order.size, multipatch.para_dim),
                                dtype=np.int32
                        )
                )
        )
