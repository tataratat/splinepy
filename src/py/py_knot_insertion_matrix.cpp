...
py::tuple ComputeGlobalKnotInsertionMatrix(
  ...
  spline->Core()->SplinepyCurrentProperties(degrees.data(),
                                            ...
                                            nullptr,
                                            nullptr);
  const int* degrees_ptr = degrees.data();
  ...
  for (int i{}; i < n_para_dims; i++) {
    // Compute number of cps before and after insertion
    n_cp_old[i] = old_kvs[i].size() - degrees_ptr[i] - 1;
  ...
  const std::vector<double>& knot_vector_old = old_kvs[parametric_dimension];
  ...
  for (int i_old{}, j_new{}; i_old + j_new < knot_vector_new.size();) {
    if ((knot_vector_old[i_old] < new_knots_ptr[j_new])
        || (j_new >= new_knots.size())) {
      knot_vector_new_ptr[i_old + j_new] = knot_vector_old[i_old];
      i_old++;
    } else {
  ...
  // Compute dimensionwise (or line-) matrices. These matrices
  // are very small compared to higher dimensional spline matrices.
  // To keep the signature, we create a view array
  py::array_t<double> np_knot_vector_old(
      static_cast<int>(knot_vector_old.size()),
      knot_vector_old.data(),
      py::none());
  const auto results =
      ComputeKnotInsertionMatrixAndKnotSpan(np_knot_vector_old,
                                            knot_vector_new,
...
BezierExtractionMatrices(const std::shared_ptr<splinepy::py::PySpline>& spline,
  ...
  auto create_new_knot_vector =
      [&](const std::vector<double>& old_kv,
          const int& deg,
  ...
  for (int i_pd{}; i_pd < n_para_dims; i_pd++) {
    const int max_new_knots =
        (old_kvs[i_pd].size() - (degrees_ptr[i_pd] + 1) * 2)
        * degrees_ptr[i_pd];

    py::array_t<double> new_knots(max_new_knots);
    const auto& new_kv =
        create_new_knot_vector(old_kvs[i_pd], degrees_ptr[i_pd], new_knots);
    n_patches_per_dimension.push_back((new_kv.size() - degrees_ptr[i_pd] - 2)
                                      / degrees_ptr[i_pd]);

    // Add data to list
    list_of_tuples.append(
        ComputeGlobalKnotInsertionMatrix(spline, i_pd, new_knots, tolerance));
  }

  // Lastly, compute the bezier extraction points, that correspond to the global
  // matrix.
  const auto& list_of_ids =
      splines::helpers::ExtractBezierPatchIDs(spline->Core(),
                                              degrees_ptr,
                                              n_patches_per_dimension.data());
  const int n_patches = list_of_ids.size();
  const int n_ctps_per_patch = list_of_ids[0].size();
  py::array_t<int> bezier_ctps_ids({n_patches, n_ctps_per_patch});
  int* bezier_ctps_ids_ptr = static_cast<int*>(bezier_ctps_ids.request().ptr);
  for (int i_patch{}; i_patch < n_patches; i_patch++) {
    std::copy_n(list_of_ids[i_patch].data(),
                n_ctps_per_patch,
                &bezier_ctps_ids_ptr[i_patch * n_ctps_per_patch]);
  }
...
void init_knot_insertion_matrix(py::module_& m) {
  ...
  m.def("global_knot_insertion_matrix",
        &ComputeGlobalKnotInsertionMatrix,
        py::arg("spline"),
        py::arg("para_dim"),
        py::arg("new_knots"),
        py::arg("tolerance"));
